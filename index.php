...
if($busca) { $sqlResumo .= " AND descricao LIKE :busca"; }
...
if($busca) { $sqlCount .= " AND descricao LIKE :busca"; }
$stmtCount = $pdo->prepare($sqlCount);
$stmtCount->bindValue(':inicio', $data_inicio);
$stmtCount->bindValue(':fim', $data_fim);
if($busca) { $stmtCount->bindValue(':busca', '%'.$busca.'%'); }
$stmtCount->execute();
...
if($busca) { $sqlTransacoes .= " AND t.descricao LIKE :busca"; }
...
<head>
    ...
    <style>
        ...
        .date-trigger-btn {
            ...
            background-color: #1e293b; /* Fundo sólido do painel */
            border: 1px solid #334155; /* Borda cinza azulada */
            border-radius: 8px; /* Cantos levemente arredondados */
            padding: 0 16px; height: 46px; min-width: 220px;
            cursor: pointer; transition: border-color 0.2s;
        }
        .date-trigger-btn:hover {
            border-color: #60a5fa; /* Azul no hover */
        }
        .trigger-left { display: flex; align-items: center; gap: 12px; width: 100%; }
        .trigger-icon { color: #94a3b8; font-size: 16px; }
        .trigger-input {
            background: transparent; border: none; outline: none;
            color: #fff; font-family: 'Poppins', sans-serif; font-weight: 500; font-size: 14px;
            width: 100%; cursor: pointer; text-transform: capitalize;
        }
        .trigger-arrow { color: #64748b; font-size: 12px; }
        ...
        .flatpickr-calendar {
            ...
            width: 180px !important; /* Largura fixa segura */
        ...
        .flatpickr-current-month { position: static !important; width: auto !important; padding: 0 !important; }
        .flatpickr-current-month input.cur-year { font-weight: 700 !important; font-size: 16px !important; color: #fff !important; }
        .numInputWrapper:hover { background: transparent !important; }

        /* SETAS */
        .flatpickr-prev-month, .flatpickr-next-month {
            position: static !important; height: 28px !important; width: 28px !important;
            border-radius: 6px !important; background: #1f2937 !important;
            display: flex !important; align-items: center !important; justify-content: center !important;
            margin: 0 !important; padding: 0 !important;
        }
        .flatpickr-prev-month:hover, .flatpickr-next-month:hover { background: #374151 !important; }
        .flatpickr-prev-month svg, .flatpickr-next-month svg { width: 10px !important; height: 10px !important; fill: #cbd5e1 !important; }

        /* O GRID REPARADO */
        .flatpickr-monthSelect-months {
            display: grid !important;
            grid-template-columns: repeat(3, 1fr) !important; /* 3 Colunas Exatas */
            gap: 10px !important; /* Espaço uniforme */
            width: 100% !important;
        ...
        .flatpickr-monthSelect-month {
            ...
            border-radius: 8px !important; /* Retângulo levemente arredondado */
            color: #9ca3af !important; /* Cinza */
        ...
        .btn-export { display: flex; align-items: center; gap: 10px; height: 46px; padding: 0 18px; background-color: var(--card-bg); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; color: var(--text-color); cursor: pointer; transition: 0.3s; font-family: 'Poppins', sans-serif; font-weight: 500; }
        .export-menu { display: none; position: absolute; right: 0; top: calc(100% + 10px); background-color: #1e293b; min-width: 200px; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 10px 25px rgba(0, 0, 0, 0.5); z-index: 1000; overflow: hidden; }
        .export-menu a { color: #e2e8f0; padding: 12px 16px; text-decoration: none; display: flex; align-items: center; gap: 12px; font-size: 14px; transition: 0.2s; }
        .export-menu a:hover { background-color: rgba(255, 255, 255, 0.08); color: #fff; }
        
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(5, 8, 16, 0.9); backdrop-filter: blur(5px); z-index: 3000; display: flex; align-items: center; justify-content: center; opacity: 0; visibility: hidden; transition: 0.3s; }
        .modal-overlay.active { opacity: 1; visibility: visible; }
        .modal { background: var(--bg-modal); width: 100%; max-width: 450px; border-radius: 16px; padding: 0; transform: translateY(30px); transition: 0.3s; max-height: 90vh; display: flex; flex-direction: column; border: 1px solid rgba(255,255,255,0.1); }
        .modal-overlay.active .modal { transform: translateY(0); }
        .modal-body { overflow-y: auto; padding: 30px; }

        .chart-panel { background: var(--bg-panel); border-radius: 20px; padding: 25px; border: 1px solid rgba(255,255,255,0.03); display: flex; flex-direction: column; height: 100%; min-height: 650px; overflow: hidden; }
        .chart-content-wrapper { flex: 1; display: flex; flex-direction: column; gap: 15px; margin-top: 10px; overflow: hidden; }
        .chart-wrapper { height: 260px !important; min-height: 260px; flex-shrink: 0; width: 100%; display: flex; align-items: center; justify-content: center; }
        canvas#myChart { max-height: 100% !important; width: auto !important; }
        .chart-details-list { flex-grow: 1; overflow-y: auto; padding-right: 5px; display: flex; flex-direction: column; gap: 10px; border-top: 1px solid rgba(255, 255, 255, 0.05); padding-top: 15px; }
        .chart-detail-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 15px; background: rgba(255, 255, 255, 0.02); border-radius: 12px; transition: 0.2s; flex-shrink: 0; }
        .chart-detail-item:hover { background: rgba(255, 255, 255, 0.05); transform: translateX(3px); }
        ...
        .ts-dropdown .ts-dropdown-content { max-height: 200px !important; overflow-y: auto !important; }
...
<div class="app-container">
    ...
    <main>
        <header class="dashboard-header">
            <div class="header-top">
                <div class="header-title-area">
                    ...
                    <div class="header-title"><h1>Visão Geral</h1><p>Bem-vindo de volta, Hiann.</p></div>
                </div>
        ...
        <section class="cards-grid">
            <div class="card"><div class="card-icon green"><i class="fas fa-arrow-up"></i></div><div class="card-info"><h3>Entradas</h3><p id="total-entradas"><?= formatarMoeda($resumo['entradas']) ?></p></div></div>
            <div class="card"><div class="card-icon red"><i class="fas fa-arrow-down"></i></div><div class="card-info"><h3>Saídas</h3><p id="total-saidas"><?= formatarMoeda($resumo['saidas']) ?></p></div></div>
            <div class="card"><div class="card-icon blue"><i class="fas fa-wallet"></i></div><div class="card-info"><h3>Saldo</h3><p id="total-saldo"><?= formatarMoeda($saldo) ?></p></div></div>
            <div class="card"><div class="card-icon orange"><i class="fas fa-exclamation-triangle"></i></div><div class="card-info"><h3>Pendente</h3><p id="total-pendente" class="text-warning"><?= formatarMoeda($resumo['contas_a_pagar']) ?></p></div></div>
        </section>

        <section class="content-grid">
            <div class="transactions-panel">
                <div class="panel-header"><h3>Transações Recentes</h3> <small style="color: var(--text-muted)"><?= $total_transacoes ?> registros</small></div>
                <?php if(count($transacoes) > 0): ?>
                <table>
                    <thead><tr><th>Status</th><th>Descrição</th><th>Categoria</th><th>Data</th><th>Valor</th><th>Ações</th></tr></thead>
                    <tbody>
                        <?php foreach($transacoes as $t): ?>
                        <tr>
                            <td><span class="status-badge <?= $t['status'] ?>" onclick="toggleStatus(<?= $t['id'] ?>, this)"><?= getStatusLabel($t['status']) ?></span></td>
                            <td class="desc-cell"><div class="icon-cat" style="background: <?= $t['cor_hex'] ?>20; color: <?= $t['cor_hex'] ?>"><i class="fas fa-receipt"></i></div> <?= htmlspecialchars($t['descricao']) ?></td>
                            <td><?= $t['cat_nome'] ?></td>
            ...
            <div class="chart-panel">
                <div class="panel-header" style="margin-bottom: 10px; border-bottom: none;"><h3>Movimentação por Categoria</h3></div>
                <div class="chart-content-wrapper">
                    <div class="chart-wrapper"><canvas id="myChart"></canvas></div>
                    <div class="chart-details-list custom-scroll">
                        <?php 
                        usort($chartData, function($a, $b) { return $b['total'] - $a['total']; });
                        ...
                        <?php endforeach; ?>
                        <?php if(empty($chartData)): ?><div style="height: 100%; display: flex; align-items: center; justify-content: center;"><p style="color: #64748b; font-size: 12px;">Sem dados.</p></div><?php endif; ?>
                    </div>
                </div>
            </div>
        </section>
...
<div class="modal-overlay" id="transactionModal">
    <div class="modal">
        <div class="modal-header"><h3 id="modalTitle">Nova Transação</h3><div class="close-modal" onclick="closeModal()"><i class="fas fa-times"></i></div></div>
...
<script>
    ...
    if(urlParams.get('status') === 'success') { document.addEventListener("DOMContentLoaded", () => showToast("Salvo com sucesso!", "success")); }

    document.addEventListener("DOMContentLoaded", function() {
        if(document.getElementById('inputCategoria')) {
            tomSelectInstance = new TomSelect("#inputCategoria", { create: false, sortField: { field: "text", direction: "asc" }, placeholder: "Selecione...", maxOptions: null });
        ...
        if(document.getElementById('datePicker')) {
            flatpickr("#datePicker", {
                plugins: [new monthSelectPlugin({ shorthand: true, dateFormat: "Y-m-d", altFormat: "F Y", theme: "dark" })],
                locale: "pt", altInput: true, altFormat: "F Y", disableMobile: "true", defaultDate: "<?= $data_inicio ?>",
                onChange: function(dates) {
                    if(dates[0]) {
                        let y = dates[0].getFullYear(), m = dates[0].getMonth() + 1;
        ...
        const btn = document.getElementById('btnExportar'), menu = document.getElementById('menuExportar');
        if(btn && menu) { btn.addEventListener('click', (e) => { e.stopPropagation(); menu.style.display = menu.style.display === 'block' ? 'none' : 'block'; }); document.addEventListener('click', () => menu.style.display = 'none'); }
        const selRep = document.getElementById('inputRepeticao'), inpParc = document.getElementById('inputParcelas');
        if(selRep && inpParc) { selRep.addEventListener('change', function() { inpParc.style.display = (this.value === 'parcelada' || this.value === 'fixa') ? 'block' : 'none'; if(this.value === 'unica') inpParc.value = ''; }); }
    });

    window.toggleSidebar = function() { document.getElementById('mySidebar').classList.toggle('active'); document.getElementById('myOverlay').classList.toggle('active'); }
    window.closeSidebar = function() { document.getElementById('mySidebar').classList.remove('active'); document.getElementById('myOverlay').classList.remove('active'); }
    window.openModal = function() { document.getElementById('transactionModal').classList.add('active'); document.body.style.overflow = 'hidden'; }
    window.closeModal = function() { 
        document.getElementById('transactionModal').classList.remove('active'); document.body.style.overflow = 'auto';
        document.getElementById('formTransacao').reset(); document.getElementById('modalTitle').innerText = 'Nova Transação'; document.getElementById('inputId').value = '';
        document.getElementById('typeDespesa').checked = true; updateModalTheme(); updateCategoryOptions('despesa');
        if (tomSelectInstance) tomSelectInstance.clear();
    }
    window.updateCategoryOptions = function(tipo) {
        if (!tomSelectInstance) return; tomSelectInstance.clear(); tomSelectInstance.clearOptions();
        allCategories.filter(c => c.tipo === tipo).forEach(c => tomSelectInstance.addOption({ value: c.id, text: c.text }));
        tomSelectInstance.refreshOptions(false);
    }
    window.updateModalTheme = function() {
        const isRec = document.getElementById('typeReceita').checked, btn = document.getElementById('btnSave');
        btn.classList.toggle('income-mode', isRec); btn.classList.toggle('expense-mode', !isRec);
    }
    window.editarTransacao = function(t) {
        openModal(); document.getElementById('modalTitle').innerText = 'Editar Transação'; document.getElementById('inputId').value = t.id;
        document.getElementById('inputValor').value = t.valor; document.getElementById('inputDesc').value = t.descricao;
        ...
        updateModalTheme(); updateCategoryOptions(t.tipo);
        setTimeout(() => { if(tomSelectInstance) tomSelectInstance.setValue(t.categoria_id); }, 50);
    ...
    new Chart(document.getElementById('myChart').getContext('2d'), {
        type: 'doughnut', 
        data: { labels: chartDataPHP.map(i => i.nome), datasets: [{ data: chartDataPHP.map(i => i.total), backgroundColor: chartDataPHP.map(i => i.cor_hex), borderWidth: 0, hoverOffset: 10, borderRadius: 5, cutout: '75%' }] },
        options: { responsive: true, maintainAspectRatio: false, layout: { padding: 20 }, plugins: { legend: { display: false }, tooltip: { backgroundColor: 'rgba(30, 41, 59, 0.95)', padding: 12, cornerRadius: 8, bodyFont: { family: 'Poppins' }, callbacks: { label: (c) => " " + c.raw.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'}) } } } },
        plugins: [{ id: 'textCenter', beforeDraw: function(chart) {
            var w = chart.width, h = chart.height, ctx = chart.ctx; ctx.restore();
            var fs = (h / 140).toFixed(2); ctx.font = "bold " + fs + "em Poppins"; ctx.textBaseline = "middle"; ctx.fillStyle = "#fff";
            var text = (<?= $totalGrafico ?>).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'});
            var tx = Math.round((w - ctx.measureText(text).width) / 2), ty = h / 2;
            ctx.fillText(text, tx, ty);
            ctx.font = "normal " + (fs*0.4) + "em Poppins"; ctx.fillStyle = "#94a3b8";
            var lbl = "Movimentado", lx = Math.round((w - ctx.measureText(lbl).width) / 2);
            ctx.fillText(lbl, lx, ty - 20); ctx.save();
        }}]
    });
</script>
